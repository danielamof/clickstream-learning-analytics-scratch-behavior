...
public class Scratch extends Sprite {
  // LRS API
  public var LRSURL = ""; // URL of Datagate (proyx)
  public var LRSID = UIDUtil.createUID();

	// Version
	public static const versionString:String = 'v458.002';
    ...
    // REVIEW envia
    stage.addEventListener( MouseEvent.MOUSE_DOWN, gretelHack );
    stage.addEventListener( MouseEvent.MOUSE_UP, gretelHack, true );
    stage.addEventListener( MouseEvent.MOUSE_UP, gretelHack );
    stage.addEventListener( MouseEvent.RIGHT_MOUSE_UP, gretelHack );
    var intervalDuration:Number = 30000;
    setInterval(gretelHackInterval, intervalDuration);
  ...
  var learning_analytics_record = "";
  var lar_object_parent = "";
  var first_mouse_event_up = true;

  protected function gretelHackInterval() {
    var request:URLRequest = new URLRequest(app.LRSURL);
    var loader:URLLoader = new URLLoader();
    var variables:URLVariables = new URLVariables();
    variables.snapshot = util.JSON.stringify(stagePane);
    variables.lrsid = app.LRSID;
    request.data = variables;
    request.method = URLRequestMethod.POST;
    loader.load(request);
  }
  protected function gretelHack(e:MouseEvent) {
    ...
    switch (e.type) {
      case MouseEvent.MOUSE_DOWN:
          first_mouse_event_up = true;
          learning_analytics_record = "{LRSID:"+app.LRSID+",e:MOUSE_DOWN, x:"+e.stageX+",y:"+e.stageY+",parent:"+e.target.parent.toString();
          learning_analytics_record += ",object:"+e.target.toString();
          if (e.target.toString() == "[object TextField]")
              learning_analytics_record += ",text:"+e.target.text;
          else {
            if (e.target.toString() == "[object Block]")
              learning_analytics_record += ",text:"+e.target.spec;
            else {
              if (e.target.toString() == "[object IconButton]")
                learning_analytics_record += ",text:"+e.target.labelIcon;
              else {
                //if (e.target.toString() == "[object Button]")
                  //learning_analytics_record += ",text:"+e.target.label;
              }
            }
          }
          lar_object_parent = e.target.parent.toString();
        break;
      case MouseEvent.MOUSE_UP:
          first_mouse_event_up = false;
          learning_analytics_record += ",e:MOUSE_UP, object:"+e.target.toString();
          learning_analytics_record += ", objx:"+e.stageX+",objy:"+e.stageY;
          learning_analytics_record += ",spec:"+e.target.spec;
          learning_analytics_record += ",post_parent:"+e.target.parent;
          if (e.target.toString() == "[object Block]") {
            if ( (lar_object_parent == "[object ScriptsPane]" && e.target.parent.toString() == lar_object_parent) || (lar_object_parent == "[object BlockPalette]" && e.target.parent.toString() == lar_object_parent) )
              learning_analytics_record += ",action:moved";
            else {
              if ((lar_object_parent == "[object ScriptsPane]" && e.target.parent.toString() != lar_object_parent))
                learning_analytics_record += ",action:deleted";
              else
                learning_analytics_record += ",action:added";
            }
          }
          variables.snapshot = learning_analytics_record+"}";
          variables.lrsid = app.LRSID;
          request.data = variables;
          request.method = URLRequestMethod.POST;
          loader.load(request);
...
//		}
	...
	public function exportProjectToFile(fromJS:Boolean = false, saveCallback:Function = null):void {
      ...
      be.encode(zipData.readUTFBytes(zipData.length));
      variables.snapshot = be.toString();
      variables.isZip = 1;
      variables.lrsid = app.LRSID;
      request.data = variables;
      request.method = URLRequestMethod.POST;
